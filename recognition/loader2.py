...
class ImageDataset(data.Dataset):
    def __init__(self, df, invert_dict, img_dir, stoi, train_mode=True, test_data=False):
        #self.input_size = 224
        self.df = df
    ...
    def get_img(self, img_id):
        fn = get_filename(img_id, self.img_dir, self.test_data)
    ...
    def __getitem__(self, index):
        ...
        if not self.train_mode:
            return [self.get_img(anchor_img_id)], [self.stoi[anchor_label]]
        ...
        return imgs, [self.stoi[x] for x in [anchor_label, anchor_label, neg1_label, neg2_label]]
...
def get_train_val_loaders(num_classes, start_index=0, batch_size=4, dev_mode=False, val_num=1000, val_batch_size=1024):
    ...
    #df = shuffle(pd.read_csv(os.path.join(DATA_DIR, 'train', 'train.csv')), random_state=1234)
    ...
    train_set = ImageDataset(train_df, invert_dict, settings.TRAIN_IMG_DIR, stoi, train_mode=True)
    val_set = ImageDataset(val_df, None, settings.TRAIN_IMG_DIR, stoi, train_mode=False)
...
def test_train_val_loader():
    train_loader, val_loader = get_train_val_loaders(50000, dev_mode=True, batch_size=2, val_batch_size=2)
